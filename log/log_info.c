...
static char *__get_printfTime()
{
    ...
    snprintf(cur_time, 20, "%d-%02d-%02d %02d:%02d:%02d", lpSystemTime.wYear, lpSystemTime.wMonth, lpSystemTime.wDay, lpSystemTime.wHour, lpSystemTime.wMinute, lpSystemTime.wSecond);
#endif
    return cur_time;
}
...
void write_log(LOG_OBJ_T *obj,const char *data, const char *filename, const int line,const char *func)
{
...
#endif
        printf(GREEN"time:%s(%.6lf)"BOLDBLACK"|"RESET BOLDYELLOW"PId:%d" YELLOW"(ThreadId:%lu)" BOLDBLACK"|"RESET CYAN"%s:%d" BLUE"(%s)"MAGENTA"---"RESET"%s\n",__get_printfTime(),__get_printfTime_d(), getpid(),ThreadId,filename, line,  func,data);
        fflush(stdout);
    }
    // 写到文件
    if(write_file_fp != NULL){
        ...
        ThreadId = gettid();
#endif
        sprintf(buf, "PId:%d(ThreadId:%lu)|",getpid(),ThreadId); // pid threadid
