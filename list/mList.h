...
struct _listobj{
    ...
    struct _function
    {
        ...
        int (*remove_node_at_d)(List *obj,int index); // 删除index处的节点（包含data,使用free(data)方式释放内存，要求匹配的内存申请机制才可调此方法）
        #endif
        void (*remove_node_all)(List *obj); // 删除表所有节点
        void (*remove_node_all_d)(List *obj); // 删除表所有节点（包含data,使用free(data)方式释放内存，要求匹配的内存申请机制才可调此方法）

        void (*delete)(List *obj); // 删除表对象
        void (*delete_d)(List *obj); // 删除表对象（包含data,使用free(data)方式释放内存，要求匹配的内存申请机制才可调此方法）
